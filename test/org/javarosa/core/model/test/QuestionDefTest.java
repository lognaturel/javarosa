...
//	}	
	...
	public void testReferences(){
		...
		ReferenceManager.getInstance().addRootTranslator(new RootTranslator("jr://audio/", "jr://resource/"));
		try{
			Reference r = ReferenceManager.getInstance().DeriveReference(audioURI);
			ref = r.getURI();
		...
		ReferenceManager.getInstance().addRootTranslator(new RootTranslator("jr://images/","jr://resource/"));
		q = fpi.getNextQuestion();
		fep = fpi.getFormEntryModel().getQuestionPrompt();
		String imURI = fep.getImageText();
		try{
			Reference r = ReferenceManager.getInstance().DeriveReference(imURI);
			ref = r.getURI();
